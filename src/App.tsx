...
/* ---- FORUM PAGE ---- */
function ForumPage({user,isGuest}:any){
  ...
  const createThread=async()=>{
    ...
    const t={
      ...
      author:user.name||user.username,username:user.username,
  ...
  if(activeThread) return <ThreadView thread={activeThread} user={user} onBack={()=>{setActiveThread(null);loadThreads();}}/>;

  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <h2 style={{margin:0}}>Discussion Forum</h2>
        {!isGuest&&<Btn onClick={()=>setShowNew(s=>!s)}>{showNew?"Cancel":"+ New thread"}</Btn>}
      </div>
      {isGuest&&(
        <div style={{...cardS,borderLeft:`3px solid ${C.guest}`,marginBottom:16}}>
          <div style={{fontWeight:600,fontSize:14,color:C.guest,marginBottom:4}}>Sign in to participate</div>
          <div style={{fontSize:13,color:C.muted}}>You can read threads as a guest, but sign in to create threads and reply.</div>
        </div>
      )}
      {showNew&&(
        <div style={{...cardS,marginBottom:16,borderLeft:`3px solid ${C.orange}`}}>
          ...
          <textarea style={{...inp,height:100,resize:"vertical",marginBottom:12}} value={newBody} onChange={(e:any)=>setNewBody(e.target.value)} placeholder="Share your thoughts, ask a question, or start a conversation..."/>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            <OutBtn onClick={()=>setShowNew(false)}>Cancel</OutBtn>
            <Btn onClick={createThread} disabled={!newTitle.trim()||!newBody.trim()}>Post thread</Btn>
          </div>
        </div>
      )}
      ...
      {threads.map((t:any)=>(
        <div key={t.id} style={{...cardS,cursor:"pointer"}} onClick={()=>setActiveThread(t)}>
...
function ThreadView({thread,user,onBack}:any){
  ...
  const postReply=async()=>{
    if(!newReply.trim()||isGuest)return;
    const r={
      body:newReply.trim(),
  ...
  return(
    <div>
      ...
      <h3 style={{fontSize:13,color:C.muted,textTransform:"uppercase",letterSpacing:1,margin:"0 0 12px"}}>{replies.length} replies</h3>
      ...
      {!isGuest?(
        <div style={{...cardS,marginTop:8}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <Btn onClick={postReply} disabled={!newReply.trim()}>Post reply</Btn>
          </div>
        </div>
      ):(
        <div style={{...cardS,borderLeft:`3px solid ${C.guest}`,textAlign:"center",padding:"1.5rem"}}>
          <div style={{fontSize:13,color:C.muted}}>Sign in to reply to this thread.</div>
        </div>
      )}
    </div>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter','Segoe UI',sans-serif"}}>
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"1.5rem 1rem"}}>
        ...
        {page==="forum"&&<ForumPage user={user} isGuest={isGuest}/>}
